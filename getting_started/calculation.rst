...
In this example we are calculate the CCS value for ubiquin protein with PBD Id 1ubq.

The structure may be downloaded from the Protein Data Bank::

   wget https://files.rcsb.org/download/1ubq.pdb

MassCCS support molecule file in ``PQR`` and ``XYZ`` format. 

Before the CCS calculation by the MassCCS software, the PDB structures need to convert to the PQR formart.
...
PDB2PQR tool you can visit: http://pdb2pqr.sourceforge.net/.

In this case is created at PQR file in pH=4.0 to obtain a ubiquin protein with total charge +4e.

.. note::
    Before the CCS calculation by the MassCCS software, we recommend relax the PDB structures by energy minimization. 

Input parameters File
~~~~~~~~~~~~~~~~~~~~~

After that is necessary use the following input parameter file in json format:
...
We executed the simulation on CPU consisting of a Dual Intel Xeon E5-2630 v3 processor with 4 cores each and AVX2, running at 2.4 GHz.

The ouput printed on screen is:

.. code-block:: text
   :caption: printed simulation parameters
...
.. code-block:: text
   :caption: information about the target molecule
